            ...
            <form action="{{route('admin.users.update', $user->id)}}" method="POST">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-lg-4">
                            <div class="mb-3">
                                <label class="form-label" for="email">{{__('phone')}}</label>
                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="mb-3">
                                ...
                                value="{{old('backup_number', $user->backup_number)}}">
                            </div>
                            @error('phone')
                                <span class="text-danger">{{$message}}</span>
                            @enderror
                        ...
                        <div class="col-lg-4">
                            ...
                            <select class=" form-control" id="select2Multiple" name="role">
                                ...
                                <option value="unit_onwer" @selected($user->role == 'unit_onwer')>{{__('Unit Onwer')}}
                                </option>
                                <option value="broker" @selected($user->role == 'broker')>{{__('Broker')}}</option>
                                <option value="client" @selected($user->role == 'client')>{{__('Client')}}</option>
                                <option value="agency" @selected($user->role == 'agency')>{{__('Agency')}}</option>
                                <option value="employee" @selected($user->role == 'employee')>{{__('Employee')}}</option>
                            </select>
                            @error('role')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
