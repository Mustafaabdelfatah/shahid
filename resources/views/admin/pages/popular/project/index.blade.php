    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('All Popular Project And Unit') }}</h4>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('admin.popular-project.create') }}"
                        class="btn btn-primary float-end btn-sm">{{ __('Create ') }}</a>
                </div>
                <div class="card-body">
                    <form id="update-pages" action="{{ route('admin.popular-project.actions') }}" method="post">
                        @csrf
                    </form>
                        ...
                        <tbody>
                            @forelse ($popular_city as $item)
                                <tr>
                                    ...
                                    <td>
                                        @foreach ($item->popular_city_project as $item_unit)
                                            @if ($item_unit->project && $item_unit->project->trans)
                                                <span>
                                                    {{ $item_unit->project->trans->where('locale', $current_lang)->first()->title ?? '' }}
                                                </span>
                                                <br>
                                            @else
                                                N/A
                                    ...
                                    <td>
                                        <div class=" d-flex justify-content-around">
                                            @if ($item->status == 1)
                                                <a href="{{ route('admin.popular-project.update-status', $item->id) }}"
                                            ...
                                            <a href="{{ route('admin.popular-project.edit', $item->id) }}"
                                        ...
                                        @include('admin.pages.popular.project._model_delete')
                                    </td>
                                </tr>
                            @empty
                            @endforelse
                ...
                <div class="col-md-12 d-flex flex justify-content-center align-items-center">
                    {{ $popular_city->links('layouts.admin.pagination') }}
                </div>
            </div> <!-- end card -->
        </div><!-- end col-->
    </div>
