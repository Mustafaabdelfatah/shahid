    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('admin.users.create') }}" class="btn btn-primary float-end mt-4 btn-sm">{{ __('Add
                    User') }}</a>
                <form action="{{ route('admin.users.index') }}" method="get">
                    <div class="row mt-3">
                        <div class="col-md-4 mt-1">
                            <input type="text" value="{{ request()->name != '' ? request()->name : '' }}" name="name"
                                placeholder="{{ trans('Search by name') }}" class="form-control">
                        </div>
                        <div class="col-md-4 mt-1">
                            <input type="text" value="{{ request()->email != '' ? request()->email : '' }}" name="email"
                                placeholder="{{ trans('Search by email') }}" class="form-control">
                        </div>
                        <div class="col-md-4 mt-1">
                            <input type="text" value="{{ request()->phone != '' ? request()->phone : '' }}" name="phone"
                                placeholder="{{ trans('Search by mobile') }}" class="form-control">
                        </div>
                        ...
                        <div class="col-md-4 mt-1">
                            <select class="select form-control" name="status">
                                ...
                                <option value="1" {{ old('status', request()->input('status')) == 1 ? 'selected' : ''
                                    ...
                                    @lang('Active') </option>
                                <option value="0" {{ old('status', request()->input('status')) != 1 && old(
    'status',
