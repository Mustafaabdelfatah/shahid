...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            <h4 class=" page-title">{{__('Edit Admins')}}</h4>
        </div>
    </div>
</div>
<div class="row">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="name" id="name"
                                    placeholder="{{__('Name')}}" value="{{old('name',$admin->name)}}">
                            </div>
                            @error('name')
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            ...
                            <select class="select2 form-control select2-multiple" data-toggle="select2"
                                id="select2Multiple" multiple="multiple" data-placeholder="{{ __('Choose Roles') }} ..."
                                name="roles[]">
                                @foreach ($roles as $role)
                                <option @selected(@$admin->roles != null ?
                                    @$admin->roles->pluck('id')->contains($role->id) : false || in_array($role->id,
                                    old('roles') ?? [])) value="{{ $role->name }}">
                                    {{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('roles')
