        ...
        <div class="card">
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-md-4">
                    <a href="{{ route('admin.categories.index') }}" class="btn btn-primary float-end">{{ __('Back') }}</a>
                    </div>
                </div>
            </div>
                                                ...
                                                <div class="row mb-3">
                                                    <div class="mb-3">
                                                        <label class="form-label"
                                                            for="title{{ $index }}">{{ __('Title in ') . Illuminate\Support\Facades\Lang::get($locale) }}<span class="text-danger">*</span></label>
                                                        <input class="form-control" type="text"
                                                            name="{{ $locale }}[title]"
                                                            value="{{ old($locale . '.title', optional($category->trans()->where('locale', $locale)->first())->title) }}"
                                                            id="title{{ $index }}">
                                                    ...
                                                    <div class="mb-3">
                                                        <label class="form-label"
                                                            for="sub_title{{ $index }}">{{ __('Sub Title in ') . Illuminate\Support\Facades\Lang::get($locale) }}<span
                                                                class="text-danger">*</span></label>
                                                        <input class="form-control" type="text"
                                                            name="{{ $locale }}[sub_title]"
                                                            value="{{ old($locale . '.sub_title', optional($category->trans()->where('locale', $locale)->first())->sub_title) }}""
                                                            id="sub_title{{ $index }}">
                                                        @if ($errors->has($locale . '.sub_title'))
                                                            <span
                                                                class="text-danger">{{ $errors->first($locale . '.sub_title') }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                ...
                                <div class="accordion-item">
                                    ...
                                    <div id="panelsStayOpen-collapse" class="accordion-collapse collapse show"
                                        ...
                                        <div class="accordion-body">
                                            ...
                                            @if ($category->image)
                                            <div class="mb-3">
                                                <img src="{{ asset($category->image) }}" width="250" alt="">
                                            </div>
                                            @endif
                                            <div class="mb-3">
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input type="checkbox" class="form-check-input" id="customCheck3" @checked($category->status == 1)
                                                    name="status">
