...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            <h4 class="page-title">{{__('Edit State')}}</h4>
        </div>
    </div>
</div>
<div class="row">
        ...
        <div class="card">
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-md-4">
                    <a href="{{ route('admin.states.index') }}" class="btn btn-primary float-end">{{ __('Back') }}</a>
                    </div>
                </div>
            </div>
                                                ...
                                                <div class="row mb-3">
                                                    <div class="mb-3">
                                                        ...
                                                        <input class="form-control" type="text"
                                                            name="{{ $locale }}[title]"
                                                            value="{{ old($locale . '.title', optional($state->trans()->where('locale', $locale)->first())->title) }}"
                                                            id="title{{ $index }}">
                                        ...
                                        <div class="accordion-body">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="country_id" name="country_id">
                                                    ...
                                                    @foreach ($countries as $item)
                                                        <option value="{{ $item->id }}" @selected($item->id == $state->country_id)>{{ $item->trans->where('locale',$current_lang)->first()->title }}</option>
                                                    @endforeach
                                                </select>
                                                @error('country_id')
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input type="checkbox" class="form-check-input" id="customCheck3"
                                                    name="status" @checked($state->status == 1)>
