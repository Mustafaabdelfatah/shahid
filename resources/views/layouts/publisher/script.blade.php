...
<script>
    $(document).ready(function() {
        $('#notificationDropdown').on('click', function(e) {
            ...
            $.ajax({
                url: '{{ route("publisher.notifications.readAll") }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(response) {
                    if (response.success) {
                        ...
                        $('.notify-item').removeClass('bg-light').addClass('bg-white');
                    }
                }
            });
        });

        $('.read-noti').on('click', function(e) {
            e.preventDefault();
            var notificationId = $(this).data('id');
            var $notificationItem = $(this);

            $.ajax({
                url: "{{ route('publisher.notifications.read') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: notificationId
                ...
                success: function(response) {
                    if (response.success) {
                        $notificationItem.removeClass('bg-light').addClass('bg-white');
                    }
                }
            });
        });
